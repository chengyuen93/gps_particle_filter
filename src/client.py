...
def latlon_Conversion(latlonArray):
	global f, stringToSend
	if len(latlonArray) == 4:
		if len(latlonArray[0]) == 12 and len(latlonArray[2]) == 13:
			lat = latlonArray[0]
			latdeg = int(lat[0:2])
			latmin = float(lat[2:])
			...
			stringToSend = '%.10f %.10f %.10f' % (longitude, latitude, 0) #might need to add \n behind the E
...
if __name__ == '__main__':
	...
	fakegps_pub = rospy.Publisher('gps', String, queue_size = 10)
	try:
		while open_Serial() and not rospy.is_shutdown():
			...
			if type(dataList) is list:
				gps_pub.publish(f)
				# fakegps_pub.publish(stringToSend)
